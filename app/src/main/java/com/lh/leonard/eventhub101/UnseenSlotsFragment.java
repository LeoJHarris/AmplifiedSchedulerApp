...
public class UnseenSlotsFragment extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        if (extras != null) {
            if (extras.getString("objectid") != null) {
                remove = true;
                eventSeen = extras.getString("objectid");
            }
        }
                ...
                if (!slot.isEmpty()) {
                    ...
                    rv.addOnItemTouchListener(new RecyclerItemClickListener(v.getContext(), rv, new RecyclerItemClickListener.OnItemClickListener() {
                        ...
                        public void onItemClick(View view, int position) {
                            ...
                            Intent slotDialogIntent = new Intent(getActivity(), UnseenSlotsDialog.class);
    ...
    private class NotGoingToEvent extends AsyncTask<Void, Integer, Void> {
        ...
        protected Void doInBackground(Void... params) {
            ...
            for (int i = 0; i < person1.pendingResponseSlot.size(); i++) {

                if (person1.pendingResponseSlot.get(i).getObjectId().equals(slot.get(positionInList).getObjectId())) {
                    pos = i;
                    break;
                }
            }
            ...
            person1.pendingResponseSlot.remove(pos);
        ...
        protected Void doInBackground(Void... params) {
            if (isAdded()) {
                ...
                for (int i = 0; i < person1.pendingResponseSlot.size(); i++) {
                    ...
                    if (person1.pendingResponseSlot.get(i).getObjectId().equals(slot.get(position).getObjectId())) {
                        pos = i;
                        break;
                    }
                }

                sendsmss(slot.get(position).getPhone(), "From EventHub: " + person1.getFullname() + "  has indicated he/she is going to your " + slot.get(position).getSubject() + " event");
                ...
                Person p = Backendless.Data.of(Person.class).findById(person);

                p.addSlotGoingToSlot(slot.get(position));

                slot.remove(position);
                person = Backendless.Data.of(Person.class).save(p);
            }
            return null;
        }
